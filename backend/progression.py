...
# XP LEDGER + TIERS
# --------------------------------------------------------------------------- #
def xp_ledger(data: dict[str, Any], completed_challenges: int = 0) -> list[dict[str, Any]]:
    ...
    from daily_challenge_state (see _completed_challenge_total); the caller
    passes it so each completed daily challenge is credited exactly once. It
    defaults to 0 so the ledger stays a pure function of `data` for tests."""
...
def summary(day: str | None = None) -> dict[str, Any]:
    ...
    ledger = xp_ledger(data, _completed_challenge_total())
